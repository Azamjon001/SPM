...
import { invalidateCache } from './productsCache'; // 🚀 НОВЫЙ СУПЕР КЭШ!
import { projectId, publicAnonKey } from './supabase/info'; // 🔑 Для SMM постов
import { ramCache, realtimeManager } from './realtimeCache'; // 🚀 RAM КЭШ + REALTIME!
        ...
        setTimeout(async () => {
          try {
            const response = await fetch(
              `https://${projectId}.supabase.co/functions/v1/make-server-6a74b22c/companies/${companyId}/media${type ? `?type=${type}` : ''}`,
              {
                headers: {
