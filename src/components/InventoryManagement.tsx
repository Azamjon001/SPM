...
export default function InventoryManagement({ companyId }: InventoryManagementProps) {
  ...
  const loadProducts = async () => {
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-6a74b22c/products?company_id=${companyId}`,
        {
          headers: {
  ...
  const createLowStockNotifications = async (products: Product[]) => {
    for (const product of products) {
      try {
        await fetch(
          `https://${projectId}.supabase.co/functions/v1/make-server-6a74b22c/notifications`,
          {
            method: 'POST',
            headers: {
  ...
  const loadPriceHistory = async (productId: number) => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-6a74b22c/products/${productId}/price-history`,
        {
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`
          }
  ...
  const updateLowStockThreshold = async (productId: number, threshold: number) => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-6a74b22c/products/${productId}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
