...
export default function PaymentPage({
  ...
  const createOrder = async () => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-6a74b22c/create-order`,
        {
          method: 'POST',
          headers: {
          ...
          body: JSON.stringify({
            ...
            items: cart.map(item => ({
              ...
              markup_percent: item.markup_percent || 0, // 💰 Процент наценки
              markup_amount: item.markup_amount || 0, // 💰 Сумма наценки за 1 штуку
  ...
  const createPayment = async (method: PaymentMethod) => {
    ...
    try {
      ...
      // Создаём платёж
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-6a74b22c/create-payment`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  const checkPaymentStatus = async (checkOrderId: string) => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-6a74b22c/check-payment/${checkOrderId}`,
        {
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`
          }
