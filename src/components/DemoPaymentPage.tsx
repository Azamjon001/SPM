    ...
    try {
      ...
      for (const item of cart) {
        const response = await fetch(
          `https://${projectId}.supabase.co/functions/v1/make-server-6a74b22c/products/${item.id}`,
          {
            method: 'GET',
            headers: {
      ...
      for (const { item, product } of productsData) {
        ...
        const updateResponse = await fetch(
          ...
          const markupPercent = product.markup_percent || 30; // По умолчанию 30%
          ...
          salesItems.push({
            ...
            markup_amount: markupAmount // 🔥 ДОБАВЛЕНО: Наценка за единицу товара!
          });
      }
      ...
      if (saveCard && userPhone) {
        ...
        const saveCardResponse = await fetch(
          `https://${projectId}.supabase.co/functions/v1/make-server-6a74b22c/save-card`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
      ...
      const historyResponse = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-6a74b22c/save-payment-history`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (salesItems.length > 0) {
        ...
        const salesResponse = await fetch(
          `https://${projectId}.supabase.co/functions/v1/make-server-2907f15a/sales-history`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
