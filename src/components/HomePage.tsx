import { getProducts, getProductsPaginated, addCustomerOrder, getUserCart, saveUserCart, getUserReceipts, saveUserReceipt, getUserLikes, saveUserLikes, getOrdersByPhone, cancelOrder, searchOrderByCode } from '../utils/api';
import { useState, useEffect, useRef, useCallback } from 'react';
import { ShoppingCart, Search, Minus, Plus, Trash2, Check, Receipt, Clock, X, Heart, Camera, BadgeCheck } from 'lucide-react';
...
import { supabase } from '../utils/supabase/client'; // 🔥 Supabase Realtime Singleton!
import { 
  getCachedProducts, 
  setCachedProducts,
...
export default function HomePage({ onLogout, userName, userPhone, userCompanyId, onOpenSettings, onNavigateTo, onLikesChange, likedProductIds = [], setLikedProductIds, cart: externalCart, setCart: externalSetCart, selectedColors: externalSelectedColors, setSelectedColors: externalSetSelectedColors }: HomePageProps) {
  ...
  // Sync cart to Supabase whenever it changes (NO localStorage)
  useEffect(() => {
    if (userPhone && Object.keys(cart).length >= 0) {
      console.log('💾 [Cart Sync] Syncing cart to Supabase for:', userPhone);
      saveUserCart(userPhone, cart).catch(error => {
        console.error('❌ [Cart Sync] Failed to sync cart to Supabase:', error);
      });
    }
  }, [cart, userPhone]);
  ...
  // Save likes to Supabase (with debounce)
  useEffect(() => {
    ...
    // Sync likes to Supabase if user is logged in (with debounce)
    if (userPhone) {
      const timeoutId = setTimeout(() => {
        console.log('💾 [Likes Sync] Saving likes to Supabase...');
        saveUserLikes(userPhone, likedProductIds).catch(error => {
          console.error('❌ [Likes Sync] Failed to sync likes to Supabase:', error);
        });
  ...
  // 🛒 REALTIME: Автоматическое обновление заказов
  const { shouldRefresh: ordersRefreshTrigger } = useCustomerOrdersRealtime(userPhone);
  
  // Load cart and receipts from Supabase on mount
  useEffect(() => {
    const loadUserData = async () => {
      if (userPhone) {
        try {
          // Load cart from Supabase
          console.log('🔄 [Cart Sync] Loading cart from Supabase for:', userPhone);
          const savedCart = await getUserCart(userPhone);
          if (savedCart && Object.keys(savedCart).length > 0) {
            console.log('✅ [Cart Sync] Cart loaded from Supabase:', Object.keys(savedCart).length, 'items');
            setCart(savedCart);
          }
          
          // Load orders from Supabase (from customer_orders table, not user_order_receipts!)
          ...
          if (savedOrders && savedOrders.length > 0) {
            console.log('✅ [Orders Sync] Orders loaded from Supabase:', savedOrders.length, 'orders');
            // Convert orders from Supabase format to local format
          ...
          console.error('❌ [Sync] Failed to load user data from Supabase:', error);
        }
      }
    };
  ...
  useEffect(() => {
    ...
    // 🔥 REALTIME ПОДПИСКИ: Моментальное обновление при изменениях!
    ...
    const productsChannel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('🔥 [Realtime] Изменение в таблице products:', payload);
    ...
    // 📱 Подписка на изменения в таблице smm_posts
    const smmChannel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('🔥 [Realtime] Изменение в таблице smm_posts:', payload);
          // Моментально обновляем данные через событие!
          window.dispatchEvent(new Event('smmPostsUpdated'));
        }
      )
      .subscribe((status) => {
        console.log('📡 [Realtime] SMM subscription status:', status);
      });
    ...
    return () => {
      ...
      supabase.removeChannel(smmChannel);
    };
  ...
  const loadProducts = async (forceRefresh = false) => {
    try {
      ...
      if (shouldRefreshProducts(5) || forceRefresh || userCompanyId) {
        console.log('🔄 [HomePage] Загружаем свежие данные из Supabase...');
        ...
        console.log(`📦 [HomePage] Получено ${data.products.length} товаров из Supabase`);
        ...
        console.log('✅ [HomePage] Кэш свежий, пропускаем загрузку из Supabase');
      }
    } catch (error) {
  ...
  return (
    <div className={`min-h-screen pb-20 relative transition-colors duration-500 ${
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 z-50" onClick={() => setShowCart(false)}>
            ...
            >
              <div className="flex flex-col h-full">
                ...
                <div className="flex-1 overflow-y-auto p-6">
                  ...
                  {myOrders.length > 0 && (
                    <div className="mb-6 relative z-50">
                      <div className={`flex items-center gap-2 mb-4 transition-colors duration-500 ${
                        displayMode === 'night' ? 'text-gray-300' : 'text-gray-700'
                      }`}>
                      ...
                      <div className={`flex gap-2 mb-4 transition-colors duration-500 ${
                        displayMode === 'night' ? 'bg-slate-700' : 'bg-gray-100'
                      } rounded-lg p-1`}>
                        <button
                          onClick={() => {
                      ...
                      <div className={`mt-6 border-t pt-6 transition-colors duration-500 ${
                        displayMode === 'night' ? 'border-slate-700' : 'border-gray-200'
                      }`}></div>
                    </div>
                  )}
                  ...
                  {Object.keys(cart).length === 0 && myOrders.length === 0 ? (
                    <div className={`text-center py-12 transition-colors duration-500 ${
                      ...
                      <ShoppingCart className={`w-16 h-16 mx-auto mb-4 transition-colors duration-500 ${
                    ...
                    <div>
                      ...
                      <div className="space-y-4">
                        ...
                        {Object.entries(cart).map(([productId, quantity]) => {
                          ...
                          return (
                            <div key={productId} className={`rounded-lg p-4 transition-colors duration-500 ${
                              displayMode === 'night' ? 'bg-slate-700' : 'bg-gray-50'
                            }`}>
                              <div className="flex justify-between mb-2">
                                <div>
                                  <h3 className={`text-sm transition-colors duration-500 ${
                              ...
                              <div className="flex items-center gap-2">
                                <button
                                  ...
                                  className={`w-8 h-8 flex items-center justify-center rounded transition-colors ${
                                    displayMode === 'night' ? 'bg-slate-600 hover:bg-slate-500' : 'bg-gray-200 hover:bg-gray-300'
                                  }`}
                                >
                                ...
                                <input
                                  ...
                                  className={`w-16 text-center border rounded py-1 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors duration-500 ${
                                    displayMode === 'night' ? 'bg-slate-600 text-white border-slate-500' : 'border-gray-300'
                                  }`}
                                  min="0"
                ...
                {/* Cart Footer */}
                {Object.keys(cart).length > 0 && (
                  <div className={`border-t p-6 pb-24 transition-colors duration-500 ${
                    displayMode === 'night' ? 'bg-transparent border-slate-700' : 'bg-transparent border-gray-200'
                  }`}>
                    <div className="mb-4">
            ...
            {filteredProducts.map((product) => (
              ...
              >
                ...
                <button
                  ...
                  className={`mt-auto w-full py-2 px-4 rounded-lg transition-colors ${ 
                    product.quantity === 0 || (cart[product.id] && cart[product.id] >= product.quantity)
      ...
      {!showCart && (
        <div 
          className={`fixed bottom-24 right-4 z-30 flex items-center transition-all duration-300 ${
            floatingSearchExpanded ? 'w-72' : 'w-14'
          }`}
        >
          {floatingSearchExpanded ? (
            <div className={`w-full flex items-center gap-2 rounded-full shadow-lg transition-colors duration-500 ${
              displayMode === 'night' ? 'bg-slate-800' : 'bg-white'
            } p-2`}>
              ...
              <input
                ...
                className={`flex-1 bg-transparent focus:outline-none transition-colors duration-500 ${
                  displayMode === 'night' ? 'text-white placeholder-gray-400' : 'text-gray-900 placeholder-gray-500'
                }`}
                autoFocus
              />
              <button
                ...
                className={`flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-full transition-colors ${
                  displayMode === 'night' ? 'hover:bg-slate-700' : 'hover:bg-gray-100'
                }`}
              >
                <X className="w-4 h-4" />
              </button>
            ...
            <button
              ...
              className={`w-14 h-14 rounded-full shadow-lg flex items-center justify-center transition-all hover:scale-110 ${
                displayMode === 'night' ? 'bg-blue-600 hover:bg-blue-700' : 'bg-blue-600 hover:bg-blue-700'
              }`}
            >
