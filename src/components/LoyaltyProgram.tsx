...
export default function LoyaltyProgram({ userPhone, userName }: LoyaltyProgramProps) {
  ...
  const loadLoyaltyData = async () => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-6a74b22c/loyalty/${userPhone}`,
        {
          headers: {
  ...
  const loadTransactions = async () => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-6a74b22c/loyalty/${userPhone}/transactions`,
        {
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`
          }
  ...
  const applyPromoCode = async () => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-6a74b22c/promo-codes/validate`,
        {
          method: 'POST',
          headers: {
