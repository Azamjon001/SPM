import { Calendar, ChevronDown, CreditCard, Download, Filter, Search } from 'lucide-react';
...
export default function PaymentHistoryPanel() {
  ...
  const loadPayments = async (companyId: number) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE}/company-payments?limit=100&company_id=${companyId}`
      );
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-3 border-2 rounded-xl transition ${showFilters
