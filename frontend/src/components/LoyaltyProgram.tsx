import { Award, Crown, Gift, History, Star, TrendingUp, Zap } from 'lucide-react';
...
export default function LoyaltyProgram({ userPhone, userName }: LoyaltyProgramProps) {
  ...
  const loadTransactions = async () => {
    try {
      const response = await fetch(
        `${API_BASE}/loyalty/${userPhone}/transactions`
      );
          ...
          <div className="space-y-3">
            {transactions.slice(0, 10).map(transaction => (
              ...
              >
                ...
                <div className={`text-lg font-bold ${transaction.type === 'earned' || transaction.type === 'bonus'
                  ? 'text-green-600'
                  : 'text-red-600'
                  }`}>
