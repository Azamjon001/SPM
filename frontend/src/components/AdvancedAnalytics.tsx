import { Award, Calendar, DollarSign, Download, Package, ShoppingCart, TrendingUp } from 'lucide-react';
import { useEffect, useState } from 'react';
import { Bar, BarChart, CartesianGrid, Cell, Legend, Line, LineChart, Pie, PieChart, ResponsiveContainer, Tooltip, XAxis, YAxis } from 'recharts';
import { getSalesHistory } from '../utils/api';
...
export default function AdvancedAnalytics({ companyId }: AdvancedAnalyticsProps) {
  ...
  const loadAnalytics = async () => {
    try {
      ...
      // Загружаем историю продаж через Go API
      const sales = await getSalesHistory(companyId || 0);
