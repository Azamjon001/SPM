import { Edit2, Minus, Plus, Receipt, Save, Trash2, TrendingDown, Users, X, Zap } from 'lucide-react';
import { useEffect, useState } from 'react';
import { addCustomExpense, deleteCustomExpense, getCompanyExpenses, updateCompanyExpenses, updateCustomExpense } from '../utils/api';
...
export default function ExpensesManager({
  ...
  const loadCustomExpenses = async () => {
    try {
      ...
      const data = await getCompanyExpenses(companyId);
      setCustomExpenses(data.expenses?.all_custom_expenses || []);
    } catch (error) {
  ...
  const handleAddCustomExpense = async () => {
    try {
      ...
      await addCustomExpense(companyId, {
        name: newExpenseForm.expense_name,
        amount: parseFloat(newExpenseForm.amount),
        expense_date: new Date().toISOString(),
        description: newExpenseForm.description || undefined
      });

      setNewExpenseForm({ expense_name: '', amount: '', description: '' });
      setShowAddForm(false);
      await loadCustomExpenses();
      alert('Затрата успешно добавлена!');
    } catch (error) {
  ...
  const handleUpdateCustomExpense = async (expenseId: number) => {
    try {
      ...
      await updateCustomExpense(expenseId, { amount: newAmount });
      await loadCustomExpenses();
    } catch (error) {
            ...
            <div className="space-y-3">
              <div className="flex gap-2">
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-medium transition-all duration-200 ${operationType.employee === 'add'
                ...
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-medium transition-all duration-200 ${operationType.employee === 'subtract'
              ...
              <div className={`text-sm font-medium px-3 py-2 rounded-lg ${operationType.employee === 'add' ? 'text-red-700 bg-red-50' : 'text-green-700 bg-green-50'
                }`}>
            ...
            <div className="space-y-3">
              <div className="flex gap-2">
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-medium transition-all duration-200 ${operationType.electricity === 'add'
                      ? 'bg-gradient-to-r from-red-600 to-red-700 text-white shadow-lg scale-105'
                ...
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-medium transition-all duration-200 ${operationType.electricity === 'subtract'
                      ? 'bg-gradient-to-r from-green-600 to-green-700 text-white shadow-lg scale-105'
                      : 'bg-white text-gray-700 border-2 border-yellow-200 hover:bg-yellow-50 hover:border-yellow-300 hover:scale-105'
                    }`}
                >
              ...
              <div className={`text-sm font-medium px-3 py-2 rounded-lg ${operationType.electricity === 'add' ? 'text-red-700 bg-red-50' : 'text-green-700 bg-green-50'
                }`}>
                  ...
                  <div className="space-y-3">
                    <div className="flex gap-2">
                      <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-medium transition-all duration-200 ${operationType[customKey] === 'add'
                            ? 'bg-gradient-to-r from-red-600 to-red-700 text-white shadow-lg scale-105'
                      ...
                      <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-medium transition-all duration-200 ${operationType[customKey] === 'subtract'
                            ? 'bg-gradient-to-r from-green-600 to-green-700 text-white shadow-lg scale-105'
                            : `bg-white text-gray-700 border-2 ${colors.borderColor} ${colors.hoverBg} ${colors.hoverBorderColor} hover:scale-105`
                          }`}
                      >
                    ...
                    <div className={`text-sm font-medium px-3 py-2 rounded-lg ${operationType[customKey] === 'add' ? 'text-red-700 bg-red-50' : 'text-green-700 bg-green-50'
                      }`}>
